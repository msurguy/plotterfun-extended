import { autocontrast, postLines, sortlines, perlinNoise, syncFaceBoundaryConfig } from '../helpers.js';
...
onmessage = function (e) {
  ...
  const getPixel = autocontrast(pixData, 0.1, width, height);

  let output = [];

  if (config.Contours) output = output.concat(getContours(getPixel, config['Contour detail']));

  if (config.Hatching) output = output.concat(hatch2(getPixel, config['Hatch scale']));
