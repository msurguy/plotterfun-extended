...
onmessage = function (e) {
  ...
  imageDataRGB(pixData, 0, 0, config.width, config.height, Math.round(major / 2));
  ...
  for (let y = hm; y < config.height; y += major) {
    ...
    for (let x = hm + xOff; x < config.width; x += major) {
      ...
      let radious = ((z * hm) / 255 / 100) * factor;
      if (!diamond) {
  ...
  if (circles.length > 0) postCircles(circles);
  if (lines.length > 0) postLines(lines);
};
