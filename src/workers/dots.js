...
onmessage = function (e) {
  ...
  for (let y = 0; y < config.height - spacing; y += spacing) {
    for (let x = 0; x <= config.width - spacing; x += spacing) {
      ...
      let pixelval = getPixel(x, y);

      // We will place a dot here, randomly, with a darker value more likely to be placed
      let p = pixelval / 255.0; // convert pixel darkness to a probablilty (p)
      ...
      if (p > dotRand()) {
        if (config['Random Direction']) {
          let dir = directionRand();
          if (directionRand() > 0.5) {
            points.push([
              [x + dir * spacing, y],
              [x + spacing - dir * spacing, y + spacing],
            ]);
          } else {
            points.push([
              [x, y + dir * spacing],
              [x + spacing, y + spacing - dir * spacing],
            ]);
          }
        } else {
          points.push([
            [x + xOffset, y + yOffset],
            [x + spacing - xOffset, y + spacing - yOffset],
          ]);
        }
      }
    }
